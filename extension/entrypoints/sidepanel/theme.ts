...
//
// The same twenty lines run in the employer dashboard (dashboard/src/lib/theme.ts). The two
// are built by different toolchains and cannot import from each other; keep them in step.
//
// The dashboard applies the stored theme from an inline <script> in <head>, before the
// stylesheet, so a chosen theme never flashes the other one. An extension page cannot: its
// content security policy forbids inline script. This runs at the top of the panel's module
// instead, so a reader whose chosen theme is the opposite of their system setting may see
// one frame of the wrong one.
